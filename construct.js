    ...
    var spin = setInterval(() => {   
       //if(counter % 5 == 0){}
        if(document.getElementById("checkbox1").checked){
            if(a){
                document.querySelector('.text-overlay').classList.toggle('flip');
            }
            b.style.transform = "rotateY(" + rotationAngle + "deg)";
        }else{
            b.style.transform = "";
            console.log(threeD);
            let a = document.querySelector('.text-overlay').classList.toggle('flip');
        }
        counter ++;
    }, 2500);
    ...
    const paragraph = document.getElementById("best");
...
function translate(idTag){
    return new Promise((resolve, reject) => {
        ...
        var charArray = new Array(12).fill("A");
    ...
    function animateTranslation(headString, charArray, index, resolve){
        ...
        function updateChar(){
            ...
            if(charArray[index] == targetChar){
                ...
                document.getElementById("best").querySelector("a").innerText = result;
